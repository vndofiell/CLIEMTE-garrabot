...
def _quotex_conectar_thread(email: str, senha: str, tipo_conta: str,
    ...
    try:
        ...
        # ── Task de sincronização automática de saldo ──────────────────────────
        # Chama get_balance() a cada 5s e atualiza _QUOTEX_STATE["saldo"].
        async def _sync_saldo_loop():
            _is_demo = tipo_conta.upper() == "DEMO"
            _ultimo  = float(saldo or 0)
            _ciclo   = 0
            while True:
                try:
                    await asyncio.sleep(5)
                    _ciclo += 1
                    # Tenta primeiro ler account_balance (rápido, sem round-trip)
                    novo = 0.0
                    if client.api and client.api.account_balance:
                        bal = client.api.account_balance
                        chave = "demoBalance" if _is_demo else "liveBalance"
                        novo = float(bal.get(chave) or bal.get("demo") or bal.get("live") or 0)
                    # A cada 30s (6 ciclos), força get_balance() para garantir sincronia
                    if novo <= 0 or _ciclo % 6 == 0:
                        novo = float(await client.get_balance() or 0)
                    if novo > 0:
                        with _QUOTEX_LOCK:
                            _QUOTEX_STATE["saldo"] = novo
                        if abs(novo - _ultimo) > 0.005:
                            print(f"[Quotex] 💰 Saldo atualizado: ${novo:.2f}")
                            _ultimo = novo
                except Exception as _ex:
                    pass  # silencioso — não trava o loop principal

        loop.create_task(_sync_saldo_loop())
...
# ── Saldo ──────────────────────────────────────────────────────────────────────
async def _get_saldo_fresco(client) -> float:
    """
    Força busca de saldo atualizado:
    limpa o cache account_balance para que get_balance() aguarde
    o próximo evento WebSocket com o saldo real.
    """
    try:
        if client.api:
            # Limpa o slot de balance para forçar nova espera
            client.api.account_balance = None
            if hasattr(client.api, 'slots') and hasattr(client.api.slots, 'balance'):
                client.api.slots.balance.clear()
    except Exception:
        pass
...
def quotex_get_saldo() -> dict:
    """Consulta saldo atual da conta Quotex (sempre busca o valor real)."""
    with _QUOTEX_LOCK:
        client = _QUOTEX_STATE.get("client")
        loop   = _QUOTEX_STATE.get("loop")
    ...
    try:
        fut   = asyncio.run_coroutine_threadsafe(_get_saldo_fresco(client), loop)
        saldo = fut.result(timeout=12)
        with _QUOTEX_LOCK:
            _QUOTEX_STATE["saldo"] = float(saldo or 0)
        return {"ok": True, "saldo": float(saldo or 0), "tipo_conta": tipo}
    except Exception as e:
        # Fallback: retorna o último saldo conhecido
        with _QUOTEX_LOCK:
            ultimo = _QUOTEX_STATE.get("saldo", 0.0)
        if ultimo > 0:
            return {"ok": True, "saldo": ultimo, "tipo_conta": tipo}
...
def quotex_operar(ativo: str, direcao: str, valor: float, duracao: int) -> dict:
    ...
    try:
        ...
        # Proteção extra: garante que api.profile.offset nunca seja None
        # (pode acontecer se o WS ainda não recebeu timeOffset do servidor)
        try:
            if client.api and client.api.profile and client.api.profile.offset is None:
                client.api.profile.offset = 0
                print("[Quotex] ⚠️  profile.offset era None → forçado para 0")
        except Exception:
            pass
        ...
        try:
            resultado = _executar_buy()
        except TypeError as te:
            # Retry único: se o erro for "timedelta NoneType", força offset=0 e tenta de novo
            if "NoneType" in str(te) and "timedelta" in str(te):
                print(f"[Quotex] ⚠️  timedelta NoneType detectado — corrigindo offset e retentando...")
                try:
                    if client.api and client.api.profile:
                        client.api.profile.offset = 0
                    resultado = _executar_buy()
                except Exception as e2:
                    tb = traceback.format_exc()
                    print(f"[Quotex] ❌ Erro no retry: {e2}\n{tb}")
                    return {"ok": False, "erro": str(e2)}
            else:
                raise
        ...
        tb = traceback.format_exc()
        print(f"[Quotex] ❌ Erro em quotex_operar: {e}\n{tb}")
        return {"ok": False, "erro": str(e)}
